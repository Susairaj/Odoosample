...
class MaterialMaterial(models.Model):
    ...
    def get_material_qty(self):
        total = 0.0; issue_total = 0.0
        store = self.env['store.store'].search([('active_store', '=', True)])
        if store.id:
            for record in self.env['materials.receiving'].search([('store_id', '=', store.id)]):
                for receiveing in self.env['receiving.quantity.material'].search([('material_receive_id', '=', record.id),('material_id', '=', self.search([('name', '=',self.name)]).id)]):
                    total += receiveing.accepted_qty
            for issue_record in self.env['issues.issues'].search([('store_id', '=', store.id)]):
                for issues in self.env['issues.material'].search([('issues_issues_id', '=', issue_record.id),('material_material_id', '=', self.search([('name', '=',self.name)]).id)]):
